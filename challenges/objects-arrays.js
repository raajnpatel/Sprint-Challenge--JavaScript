...
const contactInfo = graduates.map((feName) => {
    return `${feName.first_name} ${feName.email}`;
});
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.filter((onlyUni)=> {
    return onlyUni.university.includes('Uni');
});
console.log(uni);
...
const lowerCase = zooAnimals.map((aNameLow) => {
    return aNameLow.animal_name.toLowerCase();
});
...
*/
const lowerPopulation = zooAnimals.filter((lessFive) => {
    return lessFive.population < 5;
});
